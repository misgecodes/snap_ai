...
export default function Home() {
  ...
  const handleFile = async (file?: File) => {
    ...
    setIsSheetOpen(false); setFlowState("uploading");
  ...
  return <main className="min-h-screen bg-[#f6f5f2]">
    ...
    {(isBusy || flowState === "success" || flowState === "error") && <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#18212b]/35 p-5 backdrop-blur-sm"><div className="w-full max-w-[430px] rounded-[26px] bg-white p-7 text-center shadow-2xl sm:p-9">{isBusy && <><div className="mx-auto mb-6 flex h-16 w-16 items-center justify-center rounded-full bg-[#fff0e8] text-[#f2532f]"><LoaderCircle size={29} className="animate-spin" /></div><p className="mb-2 text-[11px] font-bold uppercase tracking-[0.15em] text-[#f2532f]">{flowState === "uploading" ? "Step 1 of 2" : "Step 2 of 2"}</p><h2 className="font-serif text-3xl tracking-[-0.05em]">{flowState === "uploading" ? "Uploading receipt" : "Reading your receipt"}</h2><p className="mx-auto mt-3 max-w-xs text-[13px] leading-5 text-[#858a8b]">{flowState === "uploading" ? "Sending your image securely to Cloudinary." : "SnapAI is finding the details and category."}</p><div className="mx-auto mt-7 flex max-w-[230px] gap-1.5"><span className="h-1.5 flex-1 rounded-full bg-[#f2532f]" /><span className={`h-1.5 flex-1 rounded-full ${flowState === "processing" ? "bg-[#f2532f]" : "bg-[#eeeDE9]"}`} /></div></>}{flowState === "success" && result && <><div className="mx-auto mb-6 flex h-16 w-16 items-center justify-center rounded-full bg-[#e8f4ed] text-[#3e8b61]"><Check size={30} /></div><p className="mb-2 text-[11px] font-bold uppercase tracking-[0.15em] text-[#3e8b61]">Expense detected</p><p className="font-serif text-5xl tracking-[-0.07em]">{formatAmount(result.amount, result.currency)}</p><h2 className="mt-2 text-lg font-bold">{result.merchant || "Unknown merchant"}</h2><div className="mt-4 flex justify-center gap-2"><span className={`rounded-full px-3 py-1.5 text-[11px] font-semibold ${categoryClass(result.category)}`}>{result.category || "Uncategorized"}</span><span className="rounded-full bg-[#f1f0ed] px-3 py-1.5 text-[11px] text-[#737776]">{result.date || "Date not provided"}</span></div><p className="mt-4 text-[12px] text-[#939896]">{result.reason || "Receipt captured successfully."}</p><button onClick={resetFlow} className="mt-8 w-full rounded-xl bg-[#18212b] py-3 text-[13px] font-bold text-white hover:bg-[#263541]">Done</button></>}{flowState === "error" && <><div className="mx-auto mb-6 flex h-16 w-16 items-center justify-center rounded-full bg-[#fff0e8] text-[#f2532f]"><X size={28} /></div><h2 className="font-serif text-3xl tracking-[-0.05em]">{errorStage === "uploading" ? "Upload needs setup" : "Analysis failed"}</h2><p className="mx-auto mt-3 max-w-xs text-[13px] leading-5 text-[#858a8b]">{errorMessage || (errorStage === "uploading" ? "We couldn’t upload your receipt. Please try again." : "We uploaded your receipt, but couldn’t analyze it. Please try again.")}</p><button onClick={resetFlow} className="mt-8 w-full rounded-xl bg-[#f2532f] py-3 text-[13px] font-bold text-white hover:bg-[#dc4526]">Try again</button></>}</div></div>}
  </main>;
}